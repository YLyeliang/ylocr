...
class ResNetVd(ResNet):
    def __init__(self, depth=50,
                 ...
                 max_pool_last=True,
                 act='relu'):
        super(ResNetVd, self).__init__(depth=depth, stem_channels=stem_channels, strides=strides, dilations=dilations,
                                       act=act)
        self.block, stage_blocks = self.arch_settings[depth]
        self.stage_blocks = stage_blocks
    ...
    def make_stem(self, stem_channels):
        def stem(input_tensor):
            x = ConvBlock(input_tensor, 3, stem_channels // 2, strides=1, kernel_initializer="he_normal", act='relu',
                          name='1')
            x = ConvBlock(x, 3, stem_channels // 2, strides=1, kernel_initializer='he_normal', act='relu', name='2')
            x = ConvBlock(x, 3, stem_channels, strides=1, kernel_initializer='he_normal', act='relu', name='3')
            return x
    ...
    def __call__(self, x):
        ...
        for l, strides in enumerate(self.strides):
            x = self.block(x, 3, self.filters, stage=l + 2, block='a', dilation=self.dilations[l], strides=strides,
                           if_first=l == 0, shortcut=True, act=self.act)
            for i in range(self.stage_blocks[l] - 1):
                x = self.block(x, 3, self.filters, stage=l + 2, block=chr(ord('b') + i), dilation=self.dilations[l],
                               shortcut=False, act=self.act)
            self.filters *= 2
